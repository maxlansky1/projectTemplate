...
```{note}
Используй [памятку по коммитам](commit_style_guide.md) для грамотной работы с Git.
```
...
1. Убедись, что Notepad++ установлен и доступен в системе.

2. Открой командную строку (или Git Bash) и настрой Git так, чтобы он использовал Notepad++ в качестве редактора коммитов, выполнив следующую команду:

```bash
git config --global core.editor "notepad++ -multiInst -nosession"
```
```{admonition} Параметры
:class: hint
-multiInst позволяет запускать несколько экземпляров Notepad++.
-nosession отключает автоматическое восстановление сессий.
```
...
Когда ты добавляешь новые библиотеки в проект или обновляешь их версии, важно обновить этот файл, чтобы другие разработчики или среды, в которых будет запускаться проект, могли легко установить нужные пакеты.
