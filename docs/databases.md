# 🗄️ Подключение и настройка SQL

#TODO: улучшить диаграммы C4

В шаблоне реализована асинхронная работа с реляционными базами данных через **SQLAlchemy 2.0+**.  
В текущей версии реализована поддержка **SQLite**, но архитектура позволяет легко добавлять и другие БД.
...
Подробнее про SQLite можно [почитать на Хабре](https://habr.com/ru/articles/547448/)
```
...
.
├── alembic/
│   ├── env.py                 # Конфигурация Alembic, управление миграциями
│   └── versions/              # Сгенерированные файлы миграций
├── configs/
│   ├── settings.py            # Основные настройки приложения (AppSettings)
│   └── schemas/
│       └── database.py        # Pydantic-схемы для настроек БД
├── src/
│   └── databases/
│       ├── dao/
│       │   ├── base.py        # Базовый DAO
│       │   └── user_dao.py    # DAO для модели User
│       └── sqlite/
│           ├── core.py        # Движок, сессия, декоратор @connection
│           ├── connection.py  # Генератор сессии для FastAPI
│           ├── models/
...
| **[SQLAlchemy](https://www.sqlalchemy.org/)**   | **ORM-библиотека**                                                         | Обеспечивает **взаимодействие** с базой данных через **Python-объекты**. Использует асинхронный драйвер `aiosqlite`.     |
...
Все настройки баз данных находятся в модуле configs/schemas/database.py, который включает схему Pydantic для настройки БД. Класс `SQLiteSettings` отвечает за настройки подключения к SQLite.
Класс `DatabaseSettings` отвечает за централизованное управление настройками подключения ко всем БД. 
...
`DatabaseSettings` инициализируется через `AppSettings` в `configs/settings.py`:
...
:class: warning
Перед запуском приложения **обязательно** обновите файл .env в корне проекта и укажите в нём все необходимые переменные окружения, необходимые для настройки базы данных.
...
Папка `data/` создаётся автоматически в корне проекта при запуске приложения, если она не существует.  
...
SQLAlchemy — это мощная **ORM-библиотека** для Python. Она предоставляет гибкий и эффективный способ работы с реляционными базами данных, позволяя разработчикам использовать **объектно-ориентированный подход** вместо написания "голого" SQL, что означает:
...
Пример добавления нового поля в класс модели:
...
```{admonition} Совет: Организация Pydantic-схем
:class: tip
...
`BaseDAO.add()` создаёт экземпляр модели, добавляет его в сессию, фиксирует изменения и возвращает созданный объект.
...
```{admonition} Совет: Валидация при создании
:class: tip
...
**Пример: массовое обновление по условию (вручную)**
...
Этот подход **не загружает объекты в память**, а формирует **прямой SQL-запрос `DELETE`**, что делает его **эффективнее** при **массовом удалении**.
...
Убедитесь, что URL в `.env` файле (и, соответственно, в `settings`) корректен.
