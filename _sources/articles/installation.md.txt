...
Python — один из самых популярных языков программирования, который используется в веб-разработке, анализе данных, машинном обучении и других областях. Ниже приведены подробные инструкции по установке и настройке Python на различных операционных системах.
...
3. Для Windows автоматически предложится загрузить последнюю версию Python (например, `python-3.x.x-amd64.exe`).
4. Скачайте установочный файл.

##### Установка Python
1. Запустите скачанный файл.
2. В первом окне установщика поставьте галочку **Add Python to PATH**.
3. Нажмите **Customize installation** (или **Install Now**, если вы уверены в настройках по умолчанию).
4. Следуйте инструкциям установщика.
5. Дождитесь завершения установки.

```{tip} 
Рекомендуется выбрать путь установки без пробелов и кириллицы, например: `C:\Python312\`.
```
...
py --version
```
Если вы увидите вывод вроде `Python 3.12.2`, значит, Python установлен корректно.

```{note}
**IDLE** по умолчанию устанавливается вместе с Python.
**pip** — менеджер пакетов, устанавливается автоматически.
```
...
Большинство дистрибутивов Linux поставляются с установленным Python. Проверьте:
...
```{attention}
На некоторых системах `python` может указывать на Python 2, который устарел. Рекомендуется использовать `python3`.
```
...
Дополнительно можно установить `pip`:
...
1. Перейдите на [официальный сайт Python](https://www.python.org/)
2. Выберите вкладку **Downloads** → **macOS**
3. Скачайте последнюю версию (например, `python-3.x.x-macosx10.x.pkg`)
...
```{attention}
Обратите внимание, что `python` может указывать на Python 2. Используйте `python3` для версии 3.
```
##### Виртуальное окружение
```bash
python3 -m venv myenv
...
Установите пакеты Python с помощью:

```bash
pip install package_name
```

##### virtualenv — улучшенное управление виртуальными окружениями
...
- Следите за тем, какая версия Python используется (`python` vs `python3`).
- Используйте `pip` или `pipx` для установки пакетов.
- Рассмотрите установку **IDE**, например, **Visual Studio Code** или **PyCharm**, для удобной разработки.

🔗 Полезные ссылки:
- [Руководство по установке Python (Real Python)](https://realpython.com/installing-python/)
- [Документация по виртуальным окружениям](https://docs.python.org/3/library/venv.html)
...
````{attention}
:class: dropdown
В идеале это делается при помощи утилиты [`scp`](https://ru.wikipedia.org/wiki/SCP_(%D1%83%D1%82%D0%B8%D0%BB%D0%B8%D1%82%D0%B0)), но на Windows зачастую возникают проблемы с этой утилитой. Крайне не рекомендуется этот быстрый, удобный, но небезопасный способ:

1. Открываем консоль и переходим в папку .ssh, куда ранее сохранились ключи (обычно находится по пути `C:\Users\User\.ssh`)

2. Выполняем команду для просмотра файла (на Linux аналогичная команда)
```bash
cat your_PUBLIC_KEY.pub
```
3. Выделяем мышкой в консоли, кликаем правой кнопкой мышки - ключ скопируется в буфер обмена

4. Добавляем свой ключ согласно инструкции
````
...
Желательно в качестве значения переменных `APP_PORT` и `DOCKER_PORT` указать открытый вами ранее порт. 
```{warning}
...
```
```{note}
Очень желательно выполнять все действия под новым юзером, чтобы избежать ошибки с правами доступа. 10 шаг нужен для того, чтобы окончательно передать права на директорию 
```
...
4. Получаем chat_id
````
2. Делаем push на GitHub, получаем уведомление в телеграм

### ✅ Резюме

Теперь ваш проект полностью настроен и готов к разработке. Ты можешь:
